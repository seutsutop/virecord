    ...
    public static class FormExtensions
    {
        ...
        public static void SupportCustomTheme(this ListView lv)
        {
            if (!lv.OwnerDraw)
            {
                ...
                {
                    using (Brush brush = new SolidBrush(VIrecordResources.Theme.BackgroundColor))
                    {
                        e.Graphics.FillRectangle(brush, e.Bounds);
                    }

                    TextRenderer.DrawText(e.Graphics, e.Header.Text, e.Font, e.Bounds.LocationOffset(2, 0).SizeOffset(-4, 0), VIrecordResources.Theme.TextColor,
                        TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);

                    if (e.Bounds.Right < lv.ClientRectangle.Right)
                    {
                        using (Pen pen = new Pen(VIrecordResources.Theme.SeparatorDarkColor))
                        using (Pen pen2 = new Pen(VIrecordResources.Theme.SeparatorLightColor))
                        {
