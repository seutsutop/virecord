...
namespace VIrecord.NativeMessagingHost
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            if (args.Length > 0)
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(input))
                    {
                        ...
                        string filePath = FileHelpers.GetAbsolutePath("VIrecord.exe");
            ...
            {
                MessageBox.Show("This executable is used to receive data from browser addon and send it to VIrecord.",
                    "VIrecord NativeMessagingHost", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
}