...
namespace VIrecord.Setup
{
    internal class Program
    {
        ...
        private static string SolutionPath => Path.Combine(ParentDir, "VIrecord.sln");
        private static string BinDir => Path.Combine(ParentDir, "VIrecord", "bin", Configuration, RuntimeId);
        private static string SteamLauncherDir => Path.Combine(ParentDir, "VIrecord.Steam", "bin", Configuration);
        private static string ExecutablePath => Path.Combine(BinDir, "VIrecord.exe");

        private static string OutputDir => Path.Combine(ParentDir, "Output");
        private static string PortableOutputDir => Path.Combine(OutputDir, "VIrecord-portable");
        private static string DebugOutputDir => Path.Combine(OutputDir, "VIrecord-debug");
        private static string SteamOutputDir => Path.Combine(OutputDir, "VIrecord-Steam");
        private static string MicrosoftStoreOutputDir => Path.Combine(OutputDir, "VIrecord-MicrosoftStore");
        private static string MicrosoftStoreDebugOutputDir => Path.Combine(OutputDir, "VIrecord-MicrosoftStore-debug");

        private static string SetupDir => Path.Combine(ParentDir, "VIrecord.Setup");
        ...
        private static string SetupPath => Path.Combine(OutputDir, $"VIrecord-{AppVersion}-setup-{Platform}.exe");
        private static string PortableZipPath => Path.Combine(OutputDir, $"VIrecord-{AppVersion}-portable-{Platform}.zip");
        private static string DebugZipPath => Path.Combine(OutputDir, $"VIrecord-{AppVersion}-debug-{Platform}.zip");
        private static string SteamUpdatesDir => Path.Combine(SteamOutputDir, "Updates");
        private static string SteamZipPath => Path.Combine(OutputDir, $"VIrecord-{AppVersion}-Steam-{Platform}.zip");
        private static string MicrosoftStoreAppxPath => Path.Combine(OutputDir, $"VIrecord-{AppVersion}-MicrosoftStore-{Platform}.appx");
        private static string MicrosoftStoreDebugAppxPath => Path.Combine(OutputDir, $"VIrecord-{AppVersion}-MicrosoftStore-debug-{Platform}.appx");
        ...
        private static string FFmpegDownloadURL => $"https://github.com/VIrecord/FFmpeg/releases/download/v{FFmpegVersion}/ffmpeg-{FFmpegVersion}-win-{Platform}.zip";
        private const string RecorderDevicesVersion = "0.12.10";
        private static string RecorderDevicesDownloadURL = $"https://github.com/VIrecord/RecorderDevices/releases/download/v{RecorderDevicesVersion}/recorder-devices-{RecorderDevicesVersion}-setup.exe";
        private const string ExifToolVersion = "13.29";
        private static string ExifToolDownloadURL = $"https://github.com/VIrecord/ExifTool/releases/download/v{ExifToolVersion}/exiftool-{ExifToolVersion}-win64.zip";

        private static void Main(string[] args)
        {
            Console.WriteLine("VIrecord setup started.");
            ...
            Console.WriteLine("VIrecord setup successfully completed.");
        }
        ...
        {
            CompileISSFile("VIrecord-setup.iss");
        ...
        private static void CreateSteamFolder()
        {
            ...
            FileHelpers.CopyFiles(Path.Combine(SteamLauncherDir, "VIrecord_Launcher.exe"), SteamOutputDir);
        ...
        private static void CreateFolder(string source, string destination, SetupJobs job)
        {
            ...
            FileHelpers.CopyFiles(Path.Combine(source, "VIrecord_File_Icon.ico"), destination);
