...
using VIrecord.ImageEditor.Presentation.Controllers;
using VIrecord.ImageEditor.Presentation.Controls;
using VIrecord.ImageEditor.Presentation.Emoji;
using VIrecord.ImageEditor.Presentation.Rendering;
using VIrecord.ImageEditor.Presentation.Theming;
using VIrecord.ImageEditor.Presentation.ViewModels;
using SkiaSharp;
using System.ComponentModel;

namespace VIrecord.ImageEditor.Presentation.Views
{
    public partial class EditorView : UserControl
    {
        ...
        private ThemeVariant MapSystemTheme(PlatformColorValues? colorValues)
        {
            if (colorValues != null)
            {
                return IsLightTheme(colorValues.ThemeVariant.ToString())
                    ? ThemeManager.VIrecordLight
                    : ThemeManager.VIrecordDark;
            }
        ...
        private void ApplyAccentColor(Color startColor)
        {
            ...
            Resources["VIrecord.Color.Accent.Start"] = startColor;
            Resources["VIrecord.Color.Accent.End"] = endColor;
            Resources["VIrecord.Color.Accent.Foreground"] = foregroundColor;

            UpdateAccentBrush(ThemeManager.VIrecordDark, startColor, endColor);
            UpdateAccentBrush(ThemeManager.VIrecordLight, startColor, endColor);
            UpdateAccentForegroundBrush(ThemeManager.VIrecordDark, foregroundColor);
            UpdateAccentForegroundBrush(ThemeManager.VIrecordLight, foregroundColor);
        }

        private void UpdateAccentBrush(Avalonia.Styling.ThemeVariant theme, Color startColor, Color endColor)
        {
            if (!Resources.TryGetResource("VIrecord.Brush.Accent", theme, out object? accentBrushValue) ||
        ...
        {
            if (!Resources.TryGetResource("VIrecord.Brush.Accent.Foreground", theme, out object? accentForegroundBrushValue) ||
        ...
        private Color GetAccentForegroundColor(Color startColor, Color endColor)
        {
            ...
            double darkSwitchRatio = GetResourceDouble(
                "VIrecord.Value.Accent.Foreground.DarkSwitchRatio",
                1.75);
        ...
        private async void OnLoadFromUrlRequested(object? sender, string url)
        {
            ...
            try
            {
                ...
                httpClient.DefaultRequestHeaders.Add("User-Agent", "VIrecord");
