...
namespace VIrecord.ImageEffectsLib
{
    public partial class ImageEffectsForm : Form
    {
        ...
        public void ImportImageEffectFile(string filePath)
        {
            try
            {
                string configJson = ImageEffectPackager.ExtractPackage(filePath, HelpersOptions.VIrecordSpecialFolders["VIrecordImageEffects"]);
                        ...
                        if (PreviewImage != null)
                        {
                            ...
                            using (Image preview = ApplyEffects())
                            {
                                if (preview != null)
                                {
                                    ...
                                    Text = string.Format("VIrecord - " + Resources.ImageEffectsForm_UpdatePreview_Image_effects___Width___0___Height___1___Render_time___2__ms,
        ...
        private void GeneratePreviewImage(int padding)
        {
            if (pbResult.ClientSize.Width > 0 && pbResult.ClientSize.Height > 0)
            {
                ...
                Color backgroundColor = VIrecordResources.Theme.BackgroundColor;
                ...
                using (Graphics g = Graphics.FromImage(PreviewImage))
                {
                    ...
                    {
                        using (Bitmap logo = VIrecordResources.Logo)
                        {
        ...
        {
            if (MessageBox.Show(Resources.WouldYouLikeToClearEffects, "VIrecord - " + Resources.Confirmation, MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                ImageEffectPreset preset = GetSelectedPreset();
        ...
        private void btnPackager_Click(object sender, EventArgs e)
        {
            ...
            if (preset != null)
            {
                ...
                {
                    MessageBox.Show(Resources.PresetNameCannotBeEmpty, "VIrecord - " + Resources.MissingPresetName, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    string json = null;
                    ...
                    if (!string.IsNullOrEmpty(json))
                    {
                        using (ImageEffectPackagerForm packagerForm = new ImageEffectPackagerForm(json, preset.Name,
                            HelpersOptions.VIrecordSpecialFolders["VIrecordImageEffects"]))
                        {
