...
namespace VIrecord
{
    public partial class NewsListControl : UserControl
    {
        ...
        {
            dgvNews.BackgroundColor = VIrecordResources.Theme.BackgroundColor;
            dgvNews.DefaultCellStyle.BackColor = dgvNews.DefaultCellStyle.SelectionBackColor = VIrecordResources.Theme.BackgroundColor;
            dgvNews.DefaultCellStyle.ForeColor = dgvNews.DefaultCellStyle.SelectionForeColor = VIrecordResources.Theme.TextColor;
            dgvNews.AlternatingRowsDefaultCellStyle.BackColor = dgvNews.AlternatingRowsDefaultCellStyle.SelectionBackColor =
                ColorHelpers.LighterColor(VIrecordResources.Theme.BackgroundColor, 0.02f);
            dgvNews.GridColor = VIrecordResources.Theme.BorderColor;

            foreach (DataGridViewRow row in dgvNews.Rows)
            {
                row.Cells[2].Style.ForeColor = row.Cells[2].Style.SelectionForeColor = VIrecordResources.Theme.TextColor;
            }
        }
        ...
        private void dgvNews_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 2)
            {
                ...
                if (row.Tag is NewsItem newsItem && !string.IsNullOrEmpty(newsItem.URL))
                {
                    row.Cells[e.ColumnIndex].Style.ForeColor = row.Cells[e.ColumnIndex].Style.SelectionForeColor = VIrecordResources.Theme.TextColor;
                }
            }
