...
namespace VIrecord
{
    ...
    {
        public const string AppName = "VIrecord";
        ...
        public const VIrecordBuild Build =
#if RELEASE
            VIrecordBuild.Release;
#elif STEAM
            VIrecordBuild.Steam;
#elif MicrosoftStore
            VIrecordBuild.MicrosoftStore;
#elif DEBUG
            VIrecordBuild.Debug;
#else
            VIrecordBuild.Unknown;
#endif
        ...
        internal static VIrecordUpdateManager UpdateManager { get; private set; }
        internal static VIrecordCLIManager CLI { get; private set; }
        ...
        public static string LogsFilePath
        {
            get
            {
                ...
                string fileName = string.Format("VIrecord-Log-{0:yyyy-MM}.txt", DateTime.Now);
        ...
        private static void Main(string[] args)
        {
            ...
            CLI = new VIrecordCLIManager(args);
            CLI.ParseCommands();

#if STEAM
            if (CheckUninstall()) return; // Steam will run VIrecord with -Uninstall when uninstalling
#endif
            ...
            using (SingleInstanceManager singleInstanceManager = new SingleInstanceManager(MutexName, PipeName, !MultiInstance, args))
            {
                if (!singleInstanceManager.IsSingleInstance || singleInstanceManager.IsFirstInstance)
                {
                    ...
                    if (restartRequested)
                    {
                        DebugHelper.WriteLine("VIrecord restarting.");

                        if (restartAsAdmin)
                        {
                            TaskHelpers.RunVIrecordAsAdmin("-silent");
                        }
                        else
                        {
        ...
        private static void Run()
        {
            ...
            DebugHelper.WriteLine("VIrecord starting.");
            ...
            UpdateManager = new VIrecordUpdateManager();
        ...
        public static void CloseSequence()
        {
            if (!closeSequenceStarted)
            {
                ...
                DebugHelper.WriteLine("VIrecord closing.");

                WatchFolderManager?.Dispose();
                SettingManager.HistoryClose();
                SettingManager.SaveAllSettings();

                DebugHelper.WriteLine("VIrecord closed.");
            }
        }
            ...
            if (!Sandbox)
            {
                ...
                if (!Directory.Exists(PersonalFolder))
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        MessageBox.Show(sb.ToString(), "VIrecord - " + Resources.Error, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        CustomPersonalPath = "";
                    }
                }
            }
        ...
        public static void UpdateHelpersSpecialFolders()
        {
            ...
            specialFolders.Add("VIrecordImageEffects", ImageEffectsFolder);
            HelpersOptions.VIrecordSpecialFolders = specialFolders;
        }
                ...
                if (!path.Equals(currentPath, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    catch (UnauthorizedAccessException e)
                    {
                        ...
                        MessageBox.Show(string.Format(Resources.Program_WritePersonalPathConfig_Cant_access_to_file, PersonalPathConfigFilePath),
                            "VIrecord", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    catch (Exception e)
                    {
                        DebugHelper.WriteException(e);
