...
namespace VIrecord
{
    public static class StartupManager
    {
        ...
        public static string StartupTargetPath
        {
            ...
            {
#if STEAM
                return FileHelpers.GetAbsolutePath("../VIrecord_Launcher.exe");
#else
        ...
        public static StartupState State
        {
...
#else
                if (ShortcutHelpers.CheckShortcut(Environment.SpecialFolder.Startup, "VIrecord", StartupTargetPath))
                {
                    if (Registry.GetValue(@"HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\StartupFolder",
                        "VIrecord.lnk", null) is byte[] status && status.Length > 0 && status[0] == 3)
                    {
            ...
            set
            {
                ...
                {
                    ShortcutHelpers.SetShortcut(value == StartupState.Enabled, Environment.SpecialFolder.Startup, "VIrecord", StartupTargetPath, "-silent");
                }
                else
                {
                    throw new NotSupportedException();
