...
namespace VIrecord
{
    public static class CleanupManager
    {
        public static void Cleanup()
        {
            try
            {
                ...
                if (Program.Settings != null)
                {
                    ...
                    {
                        CleanupFolder(Program.LogsFolder, "VIrecord-Log-*.txt", keepFileCount);
                    }
                }
            }
            catch (Exception e)
        ...
        private static void CleanupAppTempFolder()
        {
            ...
            {
                string folderPath = Path.Combine(tempFolder, "VIrecord");

                if (Directory.Exists(folderPath))
                {
                    Directory.Delete(folderPath, true);

                    DebugHelper.WriteLine($"VIrecord temp folder cleaned: {folderPath}");
                }
            }
        }
    }
