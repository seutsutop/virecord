...
namespace VIrecord
{
    public class ApplicationConfig : SettingsBase<ApplicationConfig>
    {
        ...
        public bool UseWhiteVIrecordIcon = false;
        ...
        public List<VIrecordTheme> Themes = VIrecordTheme.GetDefaultThemes();
        ...
        [Category("Clipboard"), DefaultValue(false), Description("Default .NET method can't copy image with alpha channel to clipboard. When this setting is true, VIrecord copies \"PNG\" and 32 bit \"DIB\" to clipboard in order to retain image transparency.")]
        public bool UseAlternativeClipboardCopyImage { get; set; }

        [Category("Clipboard"), DefaultValue(false), Description("Default .NET method can't get image with alpha channel from clipboard. When this setting is true, VIrecord checks if clipboard contains \"PNG\" or 32 bit \"DIB\" in order to retain image transparency.")]
        ...
        [Category("Upload"), DefaultValue(100), Description("Large file size defined in MB. VIrecord will warn before uploading large files. 0 disables this feature.")]
        public int ShowLargeFileSizeWarning { get; set; }

        [Category("Paths"), DefaultValue(false), Description("When enabled VIrecord stores Uploaders configuration files per machine, e.g. UploadersConfig-MYPC.json.")]
        public bool UseMachineSpecificUploadersConfig { get; set; }

        [Category("Paths"), Description("Custom uploaders configuration path. If you have already configured this setting in another device and you are attempting to use the same location, then backup the file before configuring this setting and restore after exiting VIrecord.")]
        [Editor(typeof(DirectoryNameEditor), typeof(UITypeEditor))]
        public string CustomUploadersConfigPath { get; set; }

        [Category("Paths"), Description("Custom hotkeys configuration path. If you have already configured this setting in another device and you are attempting to use the same location, then backup the file before configuring this setting and restore after exiting VIrecord.")]
        [Editor(typeof(DirectoryNameEditor), typeof(UITypeEditor))]
        public string CustomHotkeysConfigPath { get; set; }

        [Category("Paths"), Description("Custom screenshot path (secondary location). If custom screenshot path is temporarily unavailable (e.g. network share), VIrecord will use this location (recommended to be a local path).")]
        [Editor(typeof(DirectoryNameEditor), typeof(UITypeEditor))]
