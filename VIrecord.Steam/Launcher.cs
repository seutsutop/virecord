...
namespace VIrecord.Steam
{
    public static class Launcher
    {
        private static string ContentFolderPath = Helpers.GetAbsolutePath("VIrecord");
        private static string ContentExecutablePath = Path.Combine(ContentFolderPath, "VIrecord.exe");
        ...
        private static string UpdateExecutablePath = Path.Combine(UpdateFolderPath, "VIrecord.exe");
        ...
        public static void Run(string[] args)
        {
            ...
            if (File.Exists(ContentExecutablePath))
            {
                ...
                {
                    // Don't show VIrecord main window.
                    arguments = "-silent";
                }

                RunVIrecord(arguments);

                if (IsSteamInit)
                {
                    // Reason for this workaround is because Steam only allows writing review if user is played the game at least 5 minutes.
                    // For this reason VIrecord launcher will stay on for at least 10 seconds to let users eventually reach 5 minutes play time.
                    int waitTime = 10000;
        ...
        private static bool IsVIrecordRunning()
        {
            // Check VIrecord mutex.
        ...
        private static void UpdateVIrecord()
        {
            try
            {
                ...
                // In case updating process terminate middle of it, allow launcher to repair VIrecord.
        ...
        private static void RunVIrecord(string arguments = "")
        {
            try
            {
                if (!ShowInApp)
        ...
        private static void UninstallVIrecord()
        {
            try
            {
                while (IsVIrecordRunning())
                {
                    if (MessageBox.Show("VIrecord is currently running.\r\n\r\nPlease close VIrecord and press \"Retry\" button after it is closed.", "VIrecord - Uninstaller",
